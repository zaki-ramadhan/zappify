            ...
            {cards.map((card) => {
                ...
                return (
                    <motion.figure key={card.id} variants={fadeIn("left", {motionDelay : card.id * 200})} initial="hidden" whileInView={"show"} viewport={{once: false, amount: 0.5}} className={`card relative ${card.width} ${grow} rounded-2xl overflow-hidden`}>
                            ...
                            <div className="absolute w-full h-96 bg-gradient-to-t from-black/80 to-black/0 bottom-0 left-0"></div>

                            {card.isGrow ? (
                                <div className="text-wrapper w-[20.5rem] flex justify-between items-center text-white absolute bottom-[5.5rem] left-20 font-medium">
                                ...
                                <div className="relative -left-10 -top-44 rotate-[-90deg] text-white">
