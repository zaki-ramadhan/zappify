...
const FlipCard = (props) => {
  ...
  return (
    <figure className={`flip-card relative w-full h-full group perspective-[1000px] ${rowSpan} before:${linePosition} ${isSubGrid ? "grid grid-rows-subgrid before:content-[''] before:absolute before:left-1/2 before:translate-x-1/2 before:w-[.8px] before:h-1/2 before:bg-gradient-to-b before:from-slate-800/40 before:to-slate-700/0 before:-z-20" : ""}`}>
        <div className={`flip-card-content__wrp absolute ${isSubGrid ? 'row-start-1 row-span-6' : ''} w-full h-full transition-all duration-500 [transform-style:preserve-3d] group-hover:rotate-y-180`}>
